"""
pages/7_Win_Simulator.py
--------------------------
피타고리안 승률(Pythagorean Win Expectancy) 시뮬레이터.
빌 제임스가 고안한 공식 WinPct = RS² / (RS² + RA²) 을 이용해,
득점(RS)/실점(RA)을 가정으로 조정했을 때 예상 승률이 어떻게 변하는지
게이지 차트로 직관적으로 보여준다.
"""
...
pitchers_team = team_df[team_df["def_POS"].str.contains("투수", na=False)].dropna(subset=["pit_R", "pit_W", "pit_L"])
...
if gap > 3:
    st.info(f"🍀 실제 승률이 기대승률보다 {gap:.1f}%p 높습니다 — 접전 상황에서 유독 강했거나 '운'이 따랐을 가능성이 있습니다.")
elif gap < -3:
    st.info(f"😥 실제 승률이 기대승률보다 {abs(gap):.1f}%p 낮습니다 — 득실점 차만큼 승수를 못 챙긴, 아쉬운 시즌일 수 있습니다.")
else:
...
# ---------------------------------------------------------------
# 시뮬레이터: 득점/실점을 조정하면 기대승률이 어떻게 바뀔까?
# ---------------------------------------------------------------
...
st.caption(
    f"조정 시나리오: 득점 {rs:.0f}→{sim_rs:.0f}점, 실점 {ra:.0f}→{sim_ra:.0f}점. "
    "검은 세로선은 실제 승률 위치입니다."
)
...
for t in teams:
    ...
    t_pitchers = t_df[t_df["def_POS"].str.contains("투수", na=False)].dropna(subset=["pit_R", "pit_W", "pit_L"])
    t_ra = t_pitchers["pit_R"].sum()
    t_w = t_pitchers["pit_W"].sum()
    t_l = t_pitchers["pit_L"].sum()
...
if not luck_df.empty:
    ...
    fig_luck = go.Figure(go.Bar(
        x=luck_df["gap"], y=luck_df["team"], orientation="h",
        marker_color=colors, text=[f"{v:+.1f}%p" for v in luck_df["gap"]], textposition="outside",
    ))
    ...
    st.caption("초록(+)은 득실점 대비 승수를 더 챙긴 '승운이 좋은' 팀, 빨강(-)은 그 반대입니다.")
